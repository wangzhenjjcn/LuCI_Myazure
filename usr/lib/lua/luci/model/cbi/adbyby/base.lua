...
local h=a.exec("head -1 /tmp/adbyby/data/lazy.txt | awk -F' ' '{print $3,$4}'")or""
local i=a.exec("head -1 /tmp/adbyby/data/video.txt | awk -F' ' '{print $3,$4}'")or""
local l=a.exec("grep -v '^!' /usr/share/adbyby/data/rules.txt | wc -l")
local l=a.exec("cat /usr/share/adbyby/data/user.txt | wc -l")
local n=n.readfile("/tmp/adbyby.updated")or"1970-01-01 00:00:00"
m=Map("adbyby")
...
o.description=string.format("<strong>"..translate("Last Update Checked")..":</strong> %s<br /><strong>"..translate("Lazy Rule")..":</strong>%s <br /><strong>"..translate("Video Rule")..":</strong>%s",n,h,i)
o.inputstyle="reload"
o.write=function()
a.call("rm -rf /tmp/adbyby.updated /tmp/adbyby/admd5.json && /usr/share/adbyby/adbybyupdate.sh > /tmp/adupdate.log 2>&1 &")
a.call("sleep 5")
r.redirect(d.build_url("admin","services","adbyby"))
end
