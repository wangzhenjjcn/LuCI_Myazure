...
i=Map(o,translate("ShadowSocksR Plus+ Settings"))
i:section(SimpleSection).template="shadowsocksr/status"
local a={}
n:foreach(o,"servers",function(e)
if e.alias then
a[e[".name"]]="[%s]:%s"%{string.upper(e.type),e.alias}
elseif e.server and e.server_port then
a[e[".name"]]="[%s]:%s:%s"%{string.upper(e.type),e.server,e.server_port}
end
end)
local o={}
...
for o,t in pairs(o)do e:value(t,a[t])end
e.default="nil"
e.rmempty=false
